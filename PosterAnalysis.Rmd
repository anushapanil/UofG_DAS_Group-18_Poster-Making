...
Data Justification:
This dataset comprises of 252 adult men and contains their bodyfat % along with various physical measurements. These include weight, height, and the circumference of different body areas. The process of determining body fat is a time consuming and often costly process, involving a calculation based on body density which itself is determined by comparing the body weight in air and submersion in water. For this reason it would be useful to be able to estimate body fat percentage using more practical methods and measurements.
...
Research Question:
Is it possible to measure body fat percentage using practical physical measurements?
...
# Putting this in the first column
bodyfat <- bodyfat %>%
  relocate(Index)
# The data is not in tidy format so we convert by grouping the circumferences
bodyfat_tidy <- gather(data = bodyfat,
                   key = Body_Part,
                   value = Circumference,
                   Neck:Wrist)

# We now have 2520 rows, however there are still only 252 (<500) observations so the data should
# still be valid.
glimpse(bodyfat_tidy)

# Exporting to csv.
write_csv(bodyfat_tidy,"bodyfat_tidy.csv")
```
...
bodyfat %>%
  ...
  slice(1:3)
# Index 39 is the problem point for weight - this may be some sort of weightlifter
...
bodyfat_clean <- bodyfat_clean %>%
  filter(Index != 42)
# These top two ankles appear to be outliers
bodyfat %>%
  arrange(desc(Ankle)) %>%
...
# From the summary it is clear that the bicep term is the least significant, so we drop this variable
...
ggplot(data = Rvalues, aes(x = params,y = Rs)) +
  ...
  ylim(0,1)
# Furthermore, the 95% confidence interval of the Bicep parameter contains 0, so we were
# right to remove this variable from the model.
